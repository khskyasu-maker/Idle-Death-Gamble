...
- **시뮬레이터 결과 비공개**: 로컬 CLI에서 CSV 저장을 선택해도 `results.csv`는 gitignored이며, 누적하지 않고 최신 실행 결과로 덮어씁니다.
...
- 공개 GitHub에는 시뮬레이터의 설명, 전제, 빈 결과 입력 템플릿만 둡니다. 실제 실행 결과표, CSV, 방문 판단, 개인 예산/시간 판단은 로컬 메모나 대화에서만 다룹니다.
