...
│   ├── session_transitions.py     # State transition and support-window helpers
...
- Do not invent exact hidden mechanics such as 特図1/特図2 hold queues or symbol-selection rules. Coarse approximations are allowed only when they are explicitly labeled, conservative, and surfaced through `modeling_assumptions.py`, `simplification_notes`, `confidence`, or public output fields.
...
- Keep simulation mechanics separate from presentation. Probability/state transitions belong in `simulator.py`, `session_transitions.py`, and machine definitions; store scenario assumptions belong in `store_comparison.py`; output shaping belongs in `result_*` helper modules.
