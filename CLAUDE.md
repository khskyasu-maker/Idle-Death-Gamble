...
- **Hidden mechanics**: exact 特図1/特図2 hold queues and symbol-selection constraints should not be guessed from incomplete public specs. Coarse approximations such as support-spin efficiency, right-spend variance, and effect-time variance are allowed only when labeled in `modeling_assumptions.py`, `simplification_notes`, and public output notes.
