...
- 공개 `data/`에는 런타임 판단/추천/결과를 쓰지 않는다. 공개 `docs/`에는 명시 공유한 최신 1개 집계표만 허용한다.
...
| `result_matrix_sections.py` | 매트릭스 계열 표 헤더와 섹션 출력 | 표 행 데이터 생성은 하지 않음 |
| `result_store_printers.py` | 점포 보조 비교 출력 | 점포 비교 뷰 조립은 `result_store_views.py`에 위임 |
...
| `store_comparison.py` | 같은 기종의 점포별 레이트/설치/보더 보조 비교 | 점포 추천 순위처럼 보이지 않게 출력 문맥 유지 필요 |
| `stores.py` | 라인업 로딩, 보더 환산, 미지원 후보 표시 | 보더 환산과 관찰 후보 정책이 결합됨 |
| `machines.py` / `machine_definitions/` | 호환 레지스트리와 계열별 기종 모델 | 1차 분리 완료. 이후 기기 추가는 관련 계열 모듈에 배치 |
...
이후 `result_printers.py`도 공개 API 래퍼로 줄이고 실제 출력 구현은
...
예: 123과 라쿠엔을 `각 점포의 보더+5회` 조건으로 비교한다. 이 방식은 라쿠엔 1.111円과 123 1円의 레이트 차이를 보정해 보는 보조 분석이며, 점포 추천 순위로 쓰지 않는다. HIPS와 마루한 나니와는 대화 중 비교·참조 업장으로만 두고 시뮬 점포 범위에는 넣지 않는다.
...
- 완료: 매트릭스 계열 표 헤더와 섹션 출력을 `result_matrix_sections.py`로 분리
...
- 공개 `data/`에는 시뮬 결과, 추천, 방문 순위, 개인 예산 판단을 쓰지 않는다.
- 공개 `docs/`에는 사용자가 명시 공유한 `latest-sim-results.*` 최신 집계표만 허용하고, 추천/방문 순위/개인 예산 판단/원시 표본은 쓰지 않는다.
