...
def print_single_result(
    ...
    print_section_header("=== 오사카 난바 실제 설치기종 1엔 파친코 체감 모의 ===", 45)
    ...
    print_result_footer(machine, 45)
...
def print_multiple_result(
    ...
    print_section_header(f"=== {iterations}회 반복 리스크 평가 결과 ===", 50)
    print_machine_context(machine, results, include_korean_name=True)
    print_session_context(results)
    print_ascii_table(
        "ASCII 핵심 요약",
        ["지표", "값", "참고"],
    ...
    print_result_footer(machine, 50)
